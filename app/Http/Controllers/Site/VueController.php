...
class VueController extends Controller
{
    public static function productArr($pro){
        ...
        return [
            ...
            'colors'=> $colors,
            'image' => @$pro->pro_image,
            'price'=>   number_format(@$pro->price) . ' تومان ',
            'old_price'=>   number_format(@$pro->old_price). ' تومان ',
            'price2'=>   @$pro->price,
        ...
        foreach($products as $key=>$row){
            ...
            if($stock ){
                ...
                if($row->stock_count > 0){
                    if($price){
                        if(intval(@$row->price_second['price']) >= $price[1] && intval(@$row->price_second['price']) <= $price[0]){
...
//        $cat_pro = ProductCategory::where('category_id',$req['category_id'])->pluck('product_id')->toArray();
        ...
        $products = $products->whereHas('categories',function($q)use($req){$q->where('id',$req['category_id']);})->where('not_found','0')->orderby('stocks','DESC')->paginate(15);

        return response()->json(['pageCount'=>$products->lastPage(),'products' => self::customProduct($products)], 200);
    }
...
//        return response()->json(['products' => $product_array->toArray()], 200);
    ...
    public function filterBrand(Request $request){
        ...
        //        $price = explode('-',str_replace(' ','',$request->priceRange));
                ...
                return response()->json(['products' => self::customProduct($products,$price)], 200);
