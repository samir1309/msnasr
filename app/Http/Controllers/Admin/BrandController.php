     ...
     //   $array = array($input);
      //  $serialized_array = serialize($array);
       ...
       // $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$brand->id);
        ...
        return Redirect::action('App\Http\Controllers\Admin\BrandController@getBrand')->with('success', 'کد مورد نظر با موفقیت اضافه شد');
    }
...
//        }
      ...
      //  $serialized_array = serialize($array);
       ...
       // $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$content->id);
        return Redirect::action('App\Http\Controllers\Admin\BrandController@getBrand')->with('success' , 'آیتم مورد نظر با موفقیت ویرایش شد');
    }
    public function getDeleteBrand($id)
    {
     ...
     //   $array = array($content);
      ...
      // $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$content->id);
        Brand::destroy($id);
        return Redirect::action('App\Http\Controllers\Admin\BrandController@getBrand')->with('success', 'کدهای مورد نظر با موفقیت حذف شدند.');
