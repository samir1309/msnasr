...
use App\Library\MakeTree as MakeTree;
...
class CategoryController extends Controller
{
    ...
    {
        $category = Category::orderby('id','DESC')->paginate(100);
    ...
    public function getAddCategory()
    {
        ...
        if (!empty($categories)) {
            ...
            $categories = array(null => 'دسته اصلی ') + MakeTree::GenerateSelect();
       } else {
           $categories = array(null => 'دسته اصلی');
       }
...
//        $input['url']=str_replace(' ', '-',$input['url']);
      ...
      //  $serialized_array = serialize($array);
       ...
       // $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$category->id);
        return Redirect::action('App\Http\Controllers\Admin\CategoryController@getCategory')->with('success', 'کد مورد نظر با موفقیت اضافه شد');
    }
...
//        $input['url']=str_replace(' ', '-',$input['url']);
        ...
        if ($request->hasFile('cover')) {
            File::delete('assets/uploads/category/' . $content->cover);
            $pathMain = "assets/uploads/category";
            $extensionf = $request->file('cover')->getClientOriginalName();
            if (true) {
                $fileName = mt_rand(100, 999)."$extensionf";
      ...
      //  $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$content->id);
        return Redirect::action('App\Http\Controllers\Admin\CategoryController@getCategory');
    }
    public function getDeleteCategory($id)
    {
      ...
      //  $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$content->id);
        Category::destroy($id);
        return Redirect::action('App\Http\Controllers\Admin\CategoryController@getCategory')->with('success', 'کدهای مورد نظر با موفقیت حذف شدند.');
