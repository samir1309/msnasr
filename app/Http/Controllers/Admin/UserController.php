...
class UserController extends Controller
{
    ...
    public function postAdd(Request $request)
    {
      ...
      //  $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$user->id);
        return Redirect::action('App\Http\Controllers\Admin\UserController@getIndex')->with('success', 'آیتم جدید اضافه شد.');
    }
...
//            $user->where('id', $id)->update($input);
        ...
        //    $array = array($input);
         //   $serialized_array = serialize($array);
           ...
           // $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$user->id);
            return Redirect::action('App\Http\Controllers\Admin\UserController@getIndex')->with('success', 'آیتم مورد نظر با موفقیت ویرایش شد.');
        }

    public function postDelete(UserRequest $request)
    {
        if (User::destroy($request->get('deleteId'))) {
            return Redirect::action('App\Http\Controllers\Admin\UserController@getIndex')
...
//        event(new LogUserEvent($role->id, 'add', Auth::id()));
        if ($role->save()) {
            return Redirect::action('App\Http\Controllers\Admin\UserController@getGroup')->with('success', 'آیتم جدید اضافه شد.');
        }
...
//        event(new LogUserEvent($role->id, 'edit', Auth::id()));
        if ($role->save()) {
            return Redirect::action('App\Http\Controllers\Admin\UserController@getGroup')->with('success', 'آیتم مورد نظر با موفقیت ویرایش شد.');
        }
...
//            event(new LogUserEvent(json_encode($request->get('deleteId')), 'delete', Auth::id()));
            return Redirect::action('App\Http\Controllers\Admin\UserController@getGroup')
                ->with('success', 'کدهای مورد نظر با موفقیت حذف شدند.');
        }
    }
...
//        event(new LogUserEvent($user->id, 'change_password', Auth::id()));
    ...
    public function postEdit2($id, Request $request)
    {
        ...
        $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$user->id);
        return Redirect::action('App\Http\Controllers\Admin\UserController@getIndex2')->with('success', 'آیتم مورد نظر با موفقیت ویرایش شد.');
    }
    ...
    public function postEditAddress($id, Request $request)
    {
        ...
        return Redirect::action('App\Http\Controllers\Admin\UserController@getAddress',$data->user->id)->with('success', 'آیتم مورد نظر با موفقیت ویرایش شد.');
    }
