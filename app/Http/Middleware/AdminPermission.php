        ...
        if ($this->auth->check()) {
            if ($this->auth->user()->admin) {
                ...
                if ($segments[0] == config('site.admin')) {
                    ...
                    if (($segmentCounter > 1) && Route::currentRouteAction() !== 'App\Http\Controllers\Admin\HomeController@getIndex') {

                        foreach (User::find($this->auth->user()->id)->roles as $role) {
...
// dd('dddd');
                    ...
                    } elseif (($segmentCounter > 0) && Route::currentRouteAction() == 'App\Http\Controllers\Admin\HomeController@getIndex') {
                        return $next($request);
                    }
                }
            }
        ...
        if ($request->ajax()) {
            ...
            if (Auth::check() and Auth::user()->admin) {
                ...
                return redirect()->action('App\Http\Controllers\Admin\LoginController@getLogin');
            }
        }
    }
